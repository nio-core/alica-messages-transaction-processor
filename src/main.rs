...
use alica_messages_tp::messages::{AlicaEngineInfoValidator, AllocationAuthorityInfoValidator, PlanTreeInfoValidator, SolverResultValidator, RoleSwitchValidator, SyncReadyValidator, SyncTalkValidator};
...
fn main() {
    ...
    let mut transaction_handler = AlicaMessageTransactionHandler::new(payload_parser);
        transaction_handler.with_validator_for("ALICA_ENGINE_INFO", Box::from(AlicaEngineInfoValidator::new()))
